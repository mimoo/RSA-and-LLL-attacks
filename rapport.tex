...
\draw [<-] (0,0) -- (0,4);
\node [below] at (0.6,0) {$f'(x_0) = 0$ over $\mathbb{Z}$};
\draw [purple,<-] (0,1) -- (1,1);
\node [right] at (1.2,1.35) {$f'(x_0) = 0 \pmod{N^m}$};
\node [right] at (1.2,.65) {$\|f'(xX)\| < \frac{N^m}{sqrt{n}}$};
\draw [purple,<-] (3,2) -- (3,4);
\draw [purple,<-] (0,0) -- (0,1);

\end{tikzpicture}

Howgrave-Graham's idea is that we need to find this $f'$ through a \textbf{linear combination} of polynomials who have $x_0$ as roots, and the more polynomials we can play with, the better it is. We will see now that it's very easy for us to create polynomials $f_i$ such that $f_i(x_0) = 0 \pmod{N^m}$. And that is the reason why we choose to find a polynomial over $N^m$ and not over $N$.

LLL reduction property is that it only does linear combination of the vectors, and gives us a bound one the shortest vector of the new basis
